{ self, ... }:
{
  flake.nixosModules.oauth2-proxy =
    ...
    in
    {
      ...
      disabledModules = [ "services/security/oauth2-proxy.nix" ];

      services.oauth2-proxy = {
        enable = true;
        clientID = "oauth2-proxy";
        clientSecretFile = config.sops.secrets."oauth2_proxy/clientSecret".path;
        email.domains = [ "*" ];

        # Run locally, Traefik will talk to it
        ...
        # IMPORTANT: "static://200" is required when using OAuth2-Proxy as a ForwardAuth middleware.
        # It tells the proxy to return an HTTP 200 OK if auth is successful, rather than trying to proxy the request itself.
        upstream = "static://200";

        cookie.secretFile = config.sops.secrets."oauth2_proxy/secretSeed".path;
        ...
        extraConfig = {
          ...
          extra-jwt-issuers = "${kanidmUrl}=oauth2-proxy";
        };
      };
    };

  flake.nixosModules.oauth2-proxy-nixpkgs =
    {
      config,
      lib,
      pkgs,
